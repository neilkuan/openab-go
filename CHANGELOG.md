# Changelog

## [v0.3.1](https://github.com/neilkuan/openab-go/compare/v0.3.0...v0.3.1) - 2026-04-11
- feat: add Telegram platform adapter by @neilkuan in https://github.com/neilkuan/openab-go/pull/23

## [v0.3.0](https://github.com/neilkuan/openab-go/compare/v0.2.1...v0.3.0) - 2026-04-11
- feat: add voice message transcription support by @neilkuan in https://github.com/neilkuan/openab-go/pull/16
- Release for v0.3.0 by @openab-go-app[bot] in https://github.com/neilkuan/openab-go/pull/17
- Release for v0.3.0 by @openab-go-app[bot] in https://github.com/neilkuan/openab-go/pull/19
- fix: remove tagPrefix to avoid double-prefix tags by @neilkuan in https://github.com/neilkuan/openab-go/pull/21

## [v0.2.1](https://github.com/neilkuan/openab-go/compare/v0.2.0...v0.2.1) - 2026-04-11
- docs: add pluggable agent backends table to README by @neilkuan in https://github.com/neilkuan/openab-go/pull/13
- docs: improve CLAUDE.md with accurate build flags and architecture details by @neilkuan in https://github.com/neilkuan/openab-go/pull/15

## [v0.2.0](https://github.com/neilkuan/openab-go/compare/v0.1.3...v0.2.0) - 2026-04-11
- feat: support Discord image attachments via ACP ImageContent blocks by @neilkuan in https://github.com/neilkuan/openab-go/pull/10
- Release for v0.2.0 by @openab-go-app[bot] in https://github.com/neilkuan/openab-go/pull/11

## [v0.1.3](https://github.com/neilkuan/openab-go/compare/v0.1.2...v0.1.3) - 2026-04-10
- chore: switch Docker build cache from GHA to registry by @neilkuan in https://github.com/neilkuan/openab-go/pull/8

## [v0.1.2](https://github.com/neilkuan/openab-go/compare/v0.1.1...v0.1.2) - 2026-04-10
- chore: optimize Dockerfile layer caching by @neilkuan in https://github.com/neilkuan/openab-go/pull/6

## [v0.1.1](https://github.com/neilkuan/openab-go/compare/v0.1.0...v0.1.1) - 2026-04-10
- fix: use commit SHA instead of version tag in binary by @neilkuan in https://github.com/neilkuan/openab-go/pull/4

## [v0.1.0](https://github.com/neilkuan/openab-go/commits/v0.1.0) - 2026-04-10
- fix: streaming duplication, UTF-8 splitting, error surfacing, and version injection by @neilkuan in https://github.com/neilkuan/openab-go/pull/2
- chore: optimize Dockerfiles and add Claude Code settings by @neilkuan in https://github.com/neilkuan/openab-go/pull/3
