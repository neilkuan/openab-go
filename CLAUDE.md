...
- **`telegram/`** — Telegram platform adapter (uses [go-telegram/bot](https://github.com/go-telegram/bot) v1 with native forum topic support)
  ...
  - `handler.go` — Message handler: command detection via entity parsing (`bot_command` at offset 0), @mention/reply-to-bot detection in groups, all messages in private chats, sender context injection (`quill.sender.v1` schema), photo download (largest PhotoSize via `GetFile`/`FileDownloadLink`) and voice/audio transcription to `.tmp/`, streaming prompt responses with a background edit-streaming goroutine (2s ticker for Telegram rate limits, 4096-char message limit). Forum topic support: `MessageThreadID` from `models.Message` threaded through all send paths. Session keys: `tg:{chatID}:{threadID}` for topic messages (forum supergroups **and** private chats with BotFather "Threaded Mode" enabled — those carry `is_topic_message=true` while `chat.is_forum` stays false, so the check is on `is_topic_message`, not `is_forum`), `tg:{chatID}` otherwise. `isForumTopic` / `topicThreadID` must agree or the command path (`/info`, `/model`, …) derives a different key from the message path and misses the live session. Voice replies sent via `SendVoice` API.
  ...
  - `htmlconv.go` — `convertToTelegramHTML` rewrites GFM agent output into Telegram's `parse_mode=HTML` subset (bold, inline code, fenced code blocks with language hint, links, and **blockquotes**). Blockquotes: consecutive `> ` lines become `<blockquote>`, with `<blockquote expandable>` for quotes longer than `blockquoteExpandableMinLines` (10); nested `>>` is flattened (Telegram forbids nesting) and the quote body recurses through `convertToTelegramHTML` so inline formatting still renders. Placeholder restore order is quotes→blocks→spans so a fenced-code placeholder nested in a quote body survives.
  - `richmsg.go` — Experimental Bot API 10.1 **Rich Messages** path (opt-in via `telegram.rich_messages`, default off). `isComplexMarkdown` routes complex replies (GFM tables, ATX headings, or >4096 chars; code blocks excluded) to `sendRichMessage`, which passes agent markdown straight into `InputRichMessage.markdown` (GFM-compatible, no object-tree conversion). go-telegram/bot has no binding (latest v1.21.0 = Bot API 10.0), so it's a raw HTTP POST `{chat_id, message_thread_id?, rich_message:{markdown}}` using the adapter's bot token (`telegramAPIBase` is a var for test stubbing). Hooks into `streamPrompt`'s final send before `markdown.ConvertTables`; on success deletes the streaming placeholder (rich message is a new message), on **any** failure falls back to the `parse_mode=HTML` path. Non-streaming only; `sendRichMessageDraft` deferred. Mirrors openabdev/openab#1106.
