...
- **`stt/`** — Speech-to-text via OpenAI-compatible Whisper API. Defines a `Transcriber` interface and `OpenAITranscriber` implementation (works with any OpenAI-compatible endpoint). Supported providers: `"openai"` (default, model `whisper-1`) and `"groq"` (model `whisper-large-v3-turbo`, free tier at `https://api.groq.com/openai/v1`). Sends `response_format=json` for reliable parsing across providers. Enabled when `stt.api_key` is set in config; injected into platform adapters that support voice messages (Discord and Telegram). Logs `🎙️ stt: transcribing voice message` on each invocation.

- **`tts/`** — Text-to-speech via OpenAI-compatible TTS API.
  ...
  - `openai.go` — `OpenAISynthesizer` implementation (works with any OpenAI-compatible endpoint). Supports built-in voices (alloy, ash, ballad, coral, echo, fable, nova, onyx, sage, shimmer, verse, marin, cedar) and custom voice IDs (`voice_xxx`). Custom voice IDs are auto-wrapped as `{"id": "voice_xxx"}` in the API request. Supports `response_format` config for audio output format (mp3/wav/opus/flac/aac). Logs `🔊 tts: synthesizing voice reply` on each invocation.
  ...
  - Supported providers: `"openai"` (default, model `tts-1`) and `"groq"` (model `playai-tts`, English/Arabic only, at `https://api.groq.com/openai/v1`). Enabled when `tts.api_key` is set in config. Default voice gender is female (`nova` for OpenAI, `fritz-playai` for Groq), configurable via `voice_gender` (OpenAI) or `voice` (direct).
